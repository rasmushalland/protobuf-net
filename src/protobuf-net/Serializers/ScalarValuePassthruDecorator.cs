...
namespace ProtoBuf.Serializers
{
    ...
    sealed class ScalarValuePassthruFakeDecorator : ProtoDecoratorBase
    {
        ...
        public override void Write(object value, ProtoWriter dest) => throw new NotSupportedException();

        public override object Read(object value, ProtoReader source) => throw new NotSupportedException();
    ...
    sealed class ScalarValuePassthruDecorator : ProtoDecoratorBase
    {
        ...
        public override void Write(object value, ProtoWriter dest)
        {
            ...
            Tail.Write(innerValue, dest);
        }

        public override object Read(object value, ProtoReader source)
        {
            Helpers.DebugAssert(value == null);
            object newValue = Tail.Read(null, source);
            if (newValue != null)
            {
                if (_toSurrogate != null)
                {
        ...
        protected override void EmitRead(Compiler.CompilerContext ctx, Compiler.Local valueFrom)
        {
            ...
            if (_toSurrogate != null)
            {
                using (Compiler.Local loc = new Compiler.Local(ctx, ctx.MapType(_toSurrogate.ReturnType)))
                {
            ...
            {
                using (Compiler.Local loc = new Compiler.Local(ctx, ctx.MapType(ExpectedType)))
                {
